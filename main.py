...
def resolve(clauses, c1, c2):
    resolved = set(c1 + c2)  # Using a set to manage literals and prevent duplicates
    original_resolved = resolved.copy()

    # Negated pairs removal
    for l1 in c1:
        for l2 in c2:
            if negate(l1, l2):
                resolved.discard(l1)
                ...
                if not resolved:
                    ...
                    return True
                elif any(redundant_check(resolved, cl) for cl in clauses):
                    return True
                return list(resolved)

    return True if resolved == original_resolved else list(resolved)
...
def main():
    ...
    validation_clause = clauses.pop()  # Remove and store the last clause
    ...
    # Add negated validation clause to KB
    for clause in validation_clause:
        negated_clause = clause[1:] if clause.startswith('~') else f'~{clause}'
        clauses.append([negated_clause])
        print(clause_number, ". ", negated_clause, " {}", sep='')
        clause_number += 1
